...
class FormOutlookAsUnreadEmail extends Component {
    constructor(props) {
        super(props);
    ...
    handleChangeFiltreEmail(event) {
        ...
        this.props.changeParam(["asUnreadEmail", "inbox", "from", event.target.value]);
    }

    handleChangeFiltreSubject(event) {
        this.setState({filtre_subject: event.target.value});
        this.props.changeParam(["asUnreadEmail", "inbox", "subject", event.target.value]);
    }
    ...
    handleChangeFiltre(event) {
        switch(event.value) {
            ...
            default: this.setState({filtre_email_selected: false, filtre_subject_selected: false}); this.props.changeParam(["asUnreadEmail", "inbox"]);break;
        }
    }
...
export default withRouter(connect(mapStateToProps)(FormOutlookAsUnreadEmail));